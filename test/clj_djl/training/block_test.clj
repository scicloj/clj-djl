...
(deftest flatten-block
  (let [config (-> (loss/l2-loss) (train/new-default-training-config) (train/opt-initializer Initializer/ONES))]
    (with-open [model (-> (model/new-instance "model")
                          (model/set-block (nn/batch-flatten-block)))
                trainer (model/new-trainer model config)]
