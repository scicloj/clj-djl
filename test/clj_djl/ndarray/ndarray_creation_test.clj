...
(deftest creation
  (let [ndm (nd/new-base-manager)]
    ...
    (let [data (-> (range 0 100) (double-array))
      ...
      ;; test 2d
      (let [data2D (into-array [data data])
            ndarray (nd/create ndm data2D)]
        (is (= ndarray (nd/stack [(nd/create ndm data) (nd/create ndm data)])))))
    ;; test boolean
    (let [ndarray (nd/create ndm (boolean-array [true false true false])
                             (nd/new-shape [2 2]))
          expected (nd/create ndm (int-array [1 0 1 0]) (nd/new-shape [2 2]))]
...
(deftest creation-with-vec
  (let [ndm (nd/new-base-manager)]
    ...
    ;; test 2d
      (let [data2D [data data]
            ndarray (nd/create ndm data2D)]
        (is (= ndarray (nd/stack [(nd/create ndm data) (nd/create ndm data)])))))
    ;; test boolean
    (let [ndarray (nd/create ndm [true false true false] [2 2])
          expected (nd/create ndm (int-array [1 0 1 0]) (nd/shape [2 2]))]
      (is (= (nd/to-type ndarray "int32" false) expected))
      (is (= ndarray (nd/to-type expected "boolean" false)))
      (is (= (nd/to-type ndarray "INT32" false) expected))
      (is (= ndarray (nd/to-type expected "BOOLEAN" false)))
    ...
    (let [ndarray1 (nd/create ndm (map int [1 0 1 0]) [2 2])
          ndarray2 (nd/create ndm (int-array [1 0 1 0]) [2 2])
          ndarray3 (nd/create ndm (int-array [1 0 1 0]) (nd/shape [2 2]))]
      (is (= ndarray1 ndarray2 ndarray3)))

    (let [ndarray1 (nd/create ndm [1 0 1 0] [2 2])
          ndarray2 (nd/create ndm (long-array [1 0 1 0]) [2 2])
          ndarray3 (nd/create ndm (long-array [1 0 1 0]) (nd/shape [2 2]))
          ndarray4 (nd/create ndm [[1 0] [1 0]])]
      (is (= ndarray1 ndarray2 ndarray3 ndarray4)))))

(deftest create-csr-matrix
  (let [ndm (nd/new-base-manager)
        expected (float-array [7 8 9])
